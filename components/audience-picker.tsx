...
export function AudiencePicker({ pods, currentMemberId, selectedPodId, onChange, privateLabel = "Just me", memberId, compact = false }: AudiencePickerProps) {
  const { colors } = useAppTheme();
  const eligiblePods = pods.filter((pod) => pod.memberIds.includes(currentMemberId) && (!memberId || pod.memberIds.includes(memberId)));
  const options = [{ id: "private", name: privateLabel, memberCount: 1 }, ...eligiblePods.map((pod) => ({ id: pod.id, name: pod.name, memberCount: pod.memberIds.length }))];
