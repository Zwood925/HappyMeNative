...
export default function TabLayout() {
  ...
  return <Tabs screenOptions={{ headerShown: false, tabBarButton: HapticTab, tabBarActiveTintColor: colors.ink, tabBarInactiveTintColor: colors.muted, tabBarLabelStyle: styles.label, tabBarItemStyle: styles.item, tabBarStyle: [styles.bar, { height: 66 + bottomPadding, paddingBottom: bottomPadding, backgroundColor: colors.surface, borderTopColor: colors.border }] }}>{tabs.map((tab) => <Tabs.Screen key={tab.name} name={tab.name} options={{ title: tab.title, tabBarIcon: ({ color, focused }) => <AppIcon name={focused ? tab.activeIcon : tab.icon} size={24} color={color} /> }} />)}</Tabs>;
}
const styles = StyleSheet.create({ bar: { paddingTop: 8, borderTopWidth: StyleSheet.hairlineWidth }, item: { paddingTop: 2 }, label: { fontSize: 14, lineHeight: 19, fontWeight: "800" } });
